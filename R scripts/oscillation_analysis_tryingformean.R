...
per_summary <- peak_analysis_tidy |>
  group_by(embryo.id, timepoint, id) |>
  mutate(raw_value = value[type == 'unadjusted'],
         raw_zscore = z_score[type == 'unadjusted']) |>
...
region_summary <- wave_summary |>
  mutate(period.subset = case_when(timepoint >= -85 & timepoint <= -30 ~ 'before',
                                   timepoint >= -120 & timepoint <= -5 ~ 'before_all',
                                   timepoint >= 0 & timepoint <= 55 ~ 'during',
                                   timepoint >= 125 & timepoint <= 180 ~ 'after',
                                   timepoint >= -120 & timepoint <= -5 ~ 'before_imm')) %>% 
  mutate(period.subset = factor(period.subset, levels = c('before', 'during', 'after', 'before_all', 'before_imm'))) %>% 
  drop_na() |>
  ...
  summarise(amplitude_value = mean(amplitude_value),
            ...
            raw_value = mean(raw_value),
            raw_zscore = mean(raw_zscore),
            period = mean(period, trim = 0.1)) |>
  pivot_longer(cols = c('amplitude_value', 'abs_amplitude_value', 'amplitude_zscore', 'abs_amplitude_zscore', 'period', 'raw_value','raw_zscore'), names_to = 'stat', values_to = 'value') |>
  ungroup() 
...
region_summary |>
  dplyr::filter(type == 'unadjusted', period.subset %in% c('before', 'before_all')) %>% 
  group_by(stat, period.subset) %>% 
  # mutate(value = value * 60 * 1000 )  %>% 
  # group_by(stat, direction, period.subset) %>% get_summary_stats() 
  t_test(value ~ direction) %>% add_significance()
...
#   group_by(stat) %>% 
#   # mutate(value = value * 60 * 1000 )  %>%
#   # group_by(direction, stat) %>% get_summary_stats(value)
...
region_summary |>
  dplyr::filter(type == 'unadjusted', period.subset == c('before', 'before_all')) %>% 
  ggplot(aes(x = direction, y = value)) +
  geom_boxplot() +
  geom_point() +
  theme_bw() +
  theme(aspect.ratio = 1, legend.position = 'none') +
  facet_grid(stat~type~period.subset, scales = 'free_y') 
...
(iso_wave_ampv <- region_summary %>% 
    dplyr::filter(stat == 'abs_amplitude_value') %>% 
    mutate(value = value * 60) %>% 
