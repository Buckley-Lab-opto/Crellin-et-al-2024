function [mean_data, median_data, std_data, n_data] = gather_angle_data(I, data, xo, yo, window_side, x, y)
    extend = @(x) x(:);
    ...
    for i = xo:window_side:size(I, 2)
        for j = yo:window_side:size(I, 1)
            ...
            mean_data(end+1) = circ_mean(data_box);
            std_data(end+1) = circ_std(data_box);
            n_data(end+1) = numel(data_box);
            median_data(end+1) = circ_median(data_box);
